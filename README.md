...
The job **`deploy-lightsail`** in [`.github/workflows/cicd.yml`](.github/workflows/cicd.yml) runs after **python** and **frontend** succeed (the **docker** job is still there to validate `Dockerfile` / Compose on CI, but deploy does not use Docker on the VM). It runs on **push to `main` or `master`** or when you **Run workflow** manually. It targets a Lightsail **VM instance** named **`Nat-Sec-Hackathon-Cyber-Dashboard`**. If that instance does not exist, the workflow creates it (Ubuntu 22.04, bundle from `LIGHTSAIL_BUNDLE_ID`, default **`nano_2_0`**, AZ **`us-east-1a`**) using first-boot [`lightsail-instance-bootstrap.sh`](.github/scripts/lightsail-instance-bootstrap.sh) to install **Python 3.12** (deadsnakes), **Node 20**, and **nginx** — the same shape as local **`pip` + `uvicorn` + `npm run build`**, not Docker. It opens TCP **22** and **80**, **rsync**s the repo to **`~/aurora`**, writes **`OPENROUTER_API_KEY`** to **`~/aurora/.env`** when set, then runs [`lightsail-remote-deploy.sh`](.github/scripts/lightsail-remote-deploy.sh) (venv, `npm ci` / `npm run build`, nginx site from [`aurora-nginx-lightsail.conf`](.github/scripts/aurora-nginx-lightsail.conf), **systemd** unit **`aurora-api`** for uvicorn on port **8000**). Open the app at **`http://<instance-public-ip>/`**.
